...
z = (c[0] * x[0] + c[1] * x[1] + c[2] * x[2] + c[3] * x[3] + c[4] * x[4] + c[5] * x[5] + c[6] * x[6] + c[7] * x[7] + c[
    8] * x[8] + c[9] * x[9] + c[10] * x[10] + c[11] * x[11] + c[12] * x[12] + c[13] * x[13] + c[14] * x[14] + c[15] * x[
         15] + c[16] * x[16] + c[17] * x[17] + c[18] * x[18] + c[19] * x[19] + c[20] * x[20] + c[21] * x[21] + c[22] *
...
# mass2 = (x[3] + x[4] +x[5] == 40)#х из строк таблицы (2 строка)
# mass2 = (x[3] + x[4] +x[5] == 30)#х из строк таблицы (2 строка)
# mass3 = (x[6] + x[7] + x[8] == 30)#х из строк таблицы (3 строка)
...
mass11 = (x[1] == 30)  # ограничение. от 1 поставщика 30 товаров второму заказчику
x_non_negative = (x >= 0)
problem = op(z, [mass1, mass2, mass3, mass4, mass5, mass6, mass7, mass8, mass9, mass10,mass11,
                 x_non_negative])  # условие не отрицательности
print(problem)
